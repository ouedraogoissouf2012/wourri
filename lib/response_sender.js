...
class ResponseSender {
    ...
    /**
     * Fallback en mode DIOULA quand l'audio de la RÉPONSE (`data.audio_url`) est
     * indisponible ou que son téléchargement échoue (fix #302).
     *
     * Avant : on retombait directement sur du TEXTE FRANÇAIS — inutile pour la
     * cible dioula souvent peu alphabétisée (c'est toute la justification produit
     * du mode audio, cf. CLAUDE.md « selon langue choisie »).
     *
     * Après : on tente d'abord l'audio d'excuse dioula PRÉ-GÉNÉRÉ (getExcuseAudio /
     * audio_cache), comme le fait déjà `sendExcuse`. Un message vocal dioula
     * compréhensible (« souci technique, réessaie ») vaut mieux qu'un texte FR
     * illisible. Le texte FR ne reste qu'en ULTIME recours (au moins l'info brute).
     *
     * Robustesse : `getExcuseAudio` peut jeter (cache/génération online) ; on
     * l'isole pour garantir le repli texte quoi qu'il arrive.
     *
     * @param {string} userNumber
     * @param {object} data - Réponse API ({response, audio_url, ...})
     */
    async _sendDioulaResponseFallback(userNumber, data) {
        // 1) Audio d'excuse dioula pré-généré (prioritaire pour la cible).
        try {
            try { await this.sock.sendPresenceUpdate("recording", userNumber); } catch (_) {}
            const excuseAudio = await this.getExcuseAudio({ kind: "unavailable", isFrench: false });
            try { await this.sock.sendPresenceUpdate("paused", userNumber); } catch (_) {}
            if (excuseAudio) {
                ...
                this.logger.info("[ENVOYE] Audio d'excuse dioula (fallback réponse, TTS indispo)");
                return;
            }
            this.logger.warn("[AUDIO DIOULA] Audio d'excuse indispo — dernier recours texte FR");
        } catch (excuseErr) {
            this.logger.warn(`[AUDIO DIOULA] Échec audio d'excuse (${excuseErr.message}) — dernier recours texte FR`);
        }

        // 2) Ultime recours : texte FR (au moins l'information brute).
        if (data.response) {
            await this.sock.sendMessage(userNumber, { text: this._frText(data.response) });
            this.logger.info("[ENVOYE] Texte FR (dernier recours, mode dioula)");
        }
    }
    ...
    async sendResponse({ data, prefs, isVoiceInput, userNumber, saveFn }) {
        ...
        //   sert peu. Fallback texte FR seulement si audio indispo (API TTS down).
            ...
            if (data.audio_url) {
                try {
                    ...
                    // fix #302 : fallback = audio d'excuse dioula (pas texte FR direct)
                    this.logger.warn(`[AUDIO DIOULA] Erreur reponse audio, fallback excuse: ${audioErr.message}`);
                    await this._sendDioulaResponseFallback(userNumber, data);
                }
            } else {
                // Pas d'audio_url (TTS dioula indispo cote API) -> fallback #302
                await this._sendDioulaResponseFallback(userNumber, data);
            }
        }
        ...
        //   choisir entre lire et écouter.
            ...
            if (data.audio_url) {
                try {
                    ...
                    await this.randomDelay(...getDelays().textToAudio);
                    await this._downloadAndSendAudio(userNumber, this._resolveAudioUrl(data.audio_url));
        ...
        if ((prefs.language === "dioula" || prefs.language === "both") && data.meta) {
            await this.randomDelay(...getDelays().feedback);
            await this.sock.sendMessage(userNumber, {
