...
function createReconnectScheduler({
    ...
    function onClose(statusCode, reasonName = "") {
        if (!isReconnectable(statusCode)) {
            ...
            // #398 : un abort (loggedOut/badSession) annule aussi toute reconnexion
            // DÉJÀ planifiée. Sans ça, un 'close' récupérable (408) armait un timer,
            // puis un 'close' non-récupérable (401) arrivant avant son tir laissait
            // le timer relancer une connexion sur une session morte.
            if (timer) {
