...
const { extractCity, isValidCity } = require("./city_resolver");
...
// ─────────────────────────────────────────────
// Feedback detection patterns (fix bug #260, #269)
// ─────────────────────────────────────────────
// Frontiere de mot pour eviter les collisions substring (cf. bug #260) :
...
//   - Apres : frontiere de mot autour de chaque mot-cle + emoji match exact.
//
// ATTENTION (#269) : `\b` reste ASCII-only en JS MEME avec le flag `u`. Les
// lettres dioula ɲ (U+0272), ɔ, ɛ ne sont pas dans [A-Za-z0-9_], donc l'ancien
// `\bɲuman\b` ne matchait JAMAIS → un agriculteur envoyant "ɲuman" seul (= bien)
// n'etait plus reconnu comme thumbsUp (regression silencieuse vs PR #264).
// Fix : vraie frontiere Unicode via lookarounds negatifs sur [\p{L}\p{N}_].
// Parite exacte avec `\b` sur l'ASCII ("bonjour" rejete, "(ok)" accepte) ET
// extension correcte aux lettres dioula.
//
// ORDRE D'EVALUATION : thumbsDown TESTE EN PREMIER (defense en profondeur) pour
// que "pas bon" gagne sur "bon", et qu'un message mixte (ex: "oui mais pas bon")
// soit classe NEGATIF. Preserve par processStep (cf. tests de desambiguisation).
const _FEEDBACK_WB = "[\\p{L}\\p{N}_]"; // frontiere de mot Unicode (lettre/chiffre/_)
const _FEEDBACK_THUMBS_DOWN_RE = new RegExp(
    `(?<!${_FEEDBACK_WB})(?:non|no|mauvais|mal|pas\\s+bon|te\\s+ɲuman)(?!${_FEEDBACK_WB})|👎`,
    "u"
);
const _FEEDBACK_THUMBS_UP_RE = new RegExp(
    `(?<!${_FEEDBACK_WB})(?:oui|yes|bien|ok|super|bon|ɲuman|numan)(?!${_FEEDBACK_WB})|👍`,
    "u"
);
...
class OnboardingMachine {
    ...
    async _handleWaitingCity(prefs, userNumber, messageText) {
        // Valider AVANT d'accepter : sans ce garde, extractCity a un fallback
        // qui renvoie toujours un mot (le dernier > 3 chars), donc "Salut",
        // "ok" ou un emoji étaient acceptés comme ville et confirmés à l'écran.
        // Si la ville n'est pas reconnue, on redemande sans avancer d'étape.
        if (!isValidCity(messageText)) {
            await sendTextWithPause(this.sock, userNumber, MSG.CITY_UNKNOWN);
            return { handled: true };
        }
