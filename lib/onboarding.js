...
class OnboardingMachine {
    ...
    async _handleChangeCommand(prefs, userNumber, command) {
        if (command === "city") {
            ...
            this.userPrefs.save();
            await this.sock.sendPresenceUpdate("paused", userNumber);
            await this.sock.sendMessage(userNumber, { text: pickMsg(MSG.CHANGE_CITY, prefs.language) });
            return { handled: true };
        }
        if (command === "language") {
            ...
            this.userPrefs.save();
            await this.sock.sendPresenceUpdate("paused", userNumber);
            await this.sock.sendMessage(userNumber, { text: pickMsg(MSG.CHANGE_LANGUAGE, prefs.language) });
            return { handled: true };
        }
        if (command === "reset") {
            ...
            this.userPrefs.save();
            await this.sock.sendPresenceUpdate("paused", userNumber);
            await this.sock.sendMessage(userNumber, { text: pickMsg(MSG.RESET, prefs.language) });
            return { handled: true };
        }
    ...
    async _handleNew(prefs, userNumber, messageText) {
        ...
        await this.sock.sendPresenceUpdate("paused", userNumber);
        await this.sock.sendMessage(userNumber, { text: MSG.WELCOME });
        return { handled: true };
    }
    ...
    async _handleWaitingCity(prefs, userNumber, messageText) {
        ...
        await this.sock.sendPresenceUpdate("paused", userNumber);
        await this.sock.sendMessage(userNumber, { text: MSG.CITY_OK(cityName) });
        return { handled: true };
    }
    ...
    async _handleWaitingLanguage(prefs, userNumber, messageText) {
        ...
        if (!language) {
            await this.sock.sendPresenceUpdate("paused", userNumber);
            await this.sock.sendMessage(userNumber, { text: MSG.LANGUAGE_UNKNOWN });
            return { handled: true };
        }
        ...
        await this.sock.sendPresenceUpdate("paused", userNumber);
        await this.sock.sendMessage(userNumber, { text: MSG.PREFS_SAVED(prefs.city, langText) });
    ...
    async _handleWaitingFeedback(prefs, userNumber, messageText, { isAudioMessage, isVoiceInput }) {
        ...
        const isThumbsUp = ["👍", "oui", "yes", "bien", "ok", "super", "bon", "ɲuman", "numan"]
            .some(k => msgLower.includes(k));
        const isThumbsDown = ["👎", "non", "no", "mauvais", "mal", "pas bon", "te ɲuman"]
            .some(k => msgLower.includes(k));
