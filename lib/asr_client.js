...
class AsrClient {
     ...
     */
    async _postAudio({ endpoint, audioBuffer, filename, language, logTag, requestId }) {
     ...
     * @param {string} [requestId] - X-Request-ID de correlation (L5a #412)
     * @returns {Promise<{text:string, likely_dioula_input:boolean, language_probability:number}|null>}
     */
    async transcribeAudio(audioBuffer, filename = "audio.ogg", requestId) {
        try {
     ...
     */
    async transcribeAudioBambara(audioBuffer, filename = "audio.ogg", requestId) {
        try {
            const data = await this._postAudio({
            ...
            return await this.transcribeAudio(audioBuffer, filename, requestId);
        }
    }
}
