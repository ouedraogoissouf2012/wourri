...
/**
 * Résout un contactRef opaque vers le JID local (HMAC irréversible).
 * Compare à temps constant. Retourne null si aucun match / secret absent.
 *
 * @param {string} contactRef - HMAC hex 64 car. reçu de Convex
 * @param {Iterable<string>} jids - clés de userPrefs.data
 * @param {string} secret - WOURI_CONTACTREF_HMAC_SECRET
 * @returns {string|null}
 */
function resolveJid(contactRef, jids, secret) {
    if (!secret || !contactRef) return null;
    const expected = Buffer.from(String(contactRef), "utf8");
    if (expected.length === 0) return null;
    for (const jid of jids || []) {
        let hash;
        try {
            hash = computeContactRef(jid, secret);
        } catch (_) {
            continue;
        }
...
module.exports = { computeContactRef, normalizeNumber, resolveJid };
