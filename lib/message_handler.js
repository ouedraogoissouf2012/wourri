...
function createMessageHandler(deps) {
    ...
    async function _processAudio(msg, userNumber, prefs) {
        ...
        try {
            ...
            // L5a #412 : correlation — l'id Baileys du message sert de X-Request-ID
            // vers le moteur (meme id que l'appel /api/chat/ dans _processComplete).
            const requestId = msg.key?.id;

            if (userLanguage === 'dioula' || userLanguage === 'both') {
                logger.info(`[AUDIO] Utilisateur en mode ${userLanguage} -> ASR Bambara`);
                transcriptionResult = await asrClient.transcribeAudioBambara(audioBuffer, 'voice_message.ogg', requestId);
            } else {
                logger.info(`[AUDIO] Utilisateur en mode ${userLanguage} -> Whisper francais`);
                transcriptionResult = await asrClient.transcribeAudio(audioBuffer, 'voice_message.ogg', requestId);
            }
        ...
        try {
            ...
            const response = await apiCircuitBreaker.execute(async () =>
                axios.post(`${apiUrl}/api/chat/`, {
                    ...
                    headers: {
                        ...authHeaders(),
                        // L5a #412 : meme X-Request-ID que l'appel ASR (id Baileys).
                        ...(msg.key?.id ? { 'X-Request-ID': msg.key.id } : {}),
                    },
                })
            );
            data = response.data;
            ...
            _fireIngest({
                ...
                errorType: apiErr instanceof CircuitOpenError ? "circuit_open" : "api_error",
            });
    ...
    return async function handleMessageUpsert({ messages }) {
        for (const msg of messages) {
            ...
            try {
                ...
                // Convex : peupler contactRef à la 1re écriture (avant STOP / chat)
                if (farmerRegister) {
                    await farmerRegister.sync(userNumber);
                }

                // L5c #412 — STOP / ARRET avant l'onboarding (onboarding.js gelé)
                if (isOptOut(messageText)) {
                    await sendTextWithPause(
                        sock,
                        userNumber,
                        pickMsg(MSG.OPTOUT_CONFIRMED, prefs.language),
                    );
                    let contactRef = null;
                    try {
                        if (hmacSecret) contactRef = computeContactRef(userNumber, hmacSecret);
                    } catch (_) { /* fail-closed : POST sauté, confirmation déjà envoyée */ }
                    await reportOptOut({
                        axios,
                        convexUrl,
                        callbackKey,
