...
def get_dashboard_data(*, days: int = 7, recent_limit: int = 12) -> dict:
    ...
    with engine.connect() as conn:
                ...
                SELECT
                    ...
                    count(*) FILTER (
                        WHERE status_code >= 400
                           OR asr_success IS FALSE
                           OR nlu_out_of_scope IS TRUE
                    ) AS errors
                FROM admin_request_metrics
                WHERE observed_at >= NOW() - make_interval(days => :days)
        ...
        top_intents = conn.execute(
            text(
                """
                SELECT intent AS label, count(*) AS count
                FROM admin_request_metrics
                WHERE observed_at >= NOW() - make_interval(days => :days)
        ...
        top_cultures = conn.execute(
            text(
                """
                SELECT culture AS label, count(*) AS count
                FROM admin_request_metrics
                WHERE observed_at >= NOW() - make_interval(days => :days)
        ...
        endpoint_counts = conn.execute(
            text(
                """
                SELECT endpoint AS label, count(*) AS count
                FROM admin_request_metrics
                WHERE observed_at >= NOW() - make_interval(days => :days)
                GROUP BY endpoint
        ...
        recent = conn.execute(
            text(
                """
                SELECT observed_at, endpoint, method, status_code, duration_ms,
                       intent, culture, source, asr_success, nlu_out_of_scope
                FROM admin_request_metrics
                WHERE observed_at >= NOW() - make_interval(days => :days)
